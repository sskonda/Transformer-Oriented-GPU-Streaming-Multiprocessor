...
`TENSOR_ARCH_TREE` uses a balanced combinational reduction tree. It has no
internal state and applies backpressure directly from `out_ready` to
`in_ready`. This mode is useful for small dimensions and latency-oriented
experiments, but the complete multiply-reduction path is combinational.
...
stability under backpressure, combinational tree mode, and reset while an
